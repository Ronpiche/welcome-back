...
export class AuthService {
    ...
    async generateSessionToken(accessToken: string): Promise<string> {
        try {
            ...
            // User photo 
            const picture: string = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSKj58CU04K4aHerw7U7yzdUscWykwEi3-eGukCONbr-Q&s';
            ...
            const token: string = await this.jwtItem._generateTokenWithValidity(payload, {
                subject: sub, audience: 'sessionToken'
            });
            return token;
        } catch (error) {
    ...
    async checkSessionToken(cookies: Record<string, any>): Promise<string> {
        try {
